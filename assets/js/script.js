...
$(document).ready(function(){
	...
	$('.dropdown-tab').click(function(e){
		e.stopPropagation();
		if($(this).hasClass('dropdown-tab--active')){
			hideDropdown();
		} else {
			showDropdown($(this));
		}
	...
	// show modal when "about-me" span is clicked
	$('.about-me').click(function(){
		modalWrapper.addClass('modal-wrapper--active');
	});

	// hide modal when "close-modal" span is clickeda
	...
	$(document).click(function(e){
		// close dropdown
		if($('.dropdown-tab--active').length && 
			!$(`.dropdown-menu:not(.hidden)`)[0].contains(e.target)){
			hideDropdown();
		}
...
function hideDropdown(){
	$('.dropdown-tab').removeClass('dropdown-tab--active');
...
(function() {
	...
	properties = {
		...
		particleColor		: 'rgba(0,115,170,.65)',
		particleRadius		: 3,
		particleCount		: 60,
		...
		for(let i in particles){
			for(let j in particles){
				...
				if(length < properties.lineLength){
					...
					ctx.lineWidth = '0.5';
					ctx.strokeStyle = `rgba(0,115,170,${opacity})`;
					ctx.beginPath();
					ctx.moveTo(x1,y1);
					ctx.lineTo(x2,y2);
					ctx.closePath();
